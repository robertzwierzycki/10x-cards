...
export class HealthService {
  ...
  private static readonly DB_TIMEOUT = 3000; // 3 seconds
  private static readonly AI_TIMEOUT = 5000; // 5 seconds
   ...
   */
  static async checkHealth(supabase: SupabaseClient): Promise<HealthCheckDTO> {
    // Execute health checks in parallel for optimal performance
    const [dbResult, aiResult] = await Promise.allSettled([this.checkDatabase(supabase), this.checkAIService()]);
   ...
   */
  private static async checkDatabase(supabase: SupabaseClient): Promise<ServiceStatus> {
    try {
